...
Runs scored are highly correlated with team wins.
...
![FOX 2024](images/yankdodge.png){width="70%"}
...
```{r, include=FALSE}
fair <- read.csv("data/fair20.csv")
fair <- subset(fair, t >= 1916)
```
...
<iframe title="Fair&#39;s presidential vote equation" aria-label="Table" id="datawrapper-chart-g7B9i" src="https://datawrapper.dwcdn.net/g7B9i/3/" scrolling="no" frameborder="0" style="width: 0; min-width: 100% !important; border: none;" height="609" data-external="1">
...
errors.fit <- rep(NA, nrow(years))
errors.fit2 <- rep(NA, nrow(years))
...
predictionerror <- 49.2 - prediction24
predictionerror
```

2024 values based on the FAIR [site](https://fairmodel.econ.yale.edu/vote2020/computv4.htm)
